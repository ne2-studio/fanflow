...
- **Input**: artist name, title, headline, description, cover image (uploaded file), CTA text, Facebook Pixel ID (required), destination links (Spotify only, MVP)
- **Output (OK)**: release id, generated slug/URL (e.g. `fanflow.app/<slug>`)
- **Errors**: `invalid_destination` — non-Spotify link (not idempotent); `invalid_facebook_pixel_id` — missing or non-numeric Pixel ID (not idempotent); `cover_image_required` — no cover image uploaded (not idempotent); `invalid_cover_image_type` — file is not JPEG/PNG/WebP (not idempotent); `cover_image_too_large` — file exceeds 10 MB (not idempotent); `slug_taken` (not idempotent)
- **Rules**: only Spotify is a valid destination in MVP; a Facebook Pixel ID is mandatory on every release — FanFlow is attribution-first, so there is no way to publish a release without one; the artist name is mandatory and is displayed on the landing page alongside the title; a cover image is mandatory and is processed server-side into a 420x420 square (center-cropped, not stretched) WebP image before storage; there is no separate background image — the landing page background is a blurred/expanded rendering of the cover image; creating a release triggers landing page generation and publish (Database → Static Generator → HTML → MinIO → Nginx); one landing page per release, auto-managed — there is no separate landing-page entity or publish action

## 2) UpdateRelease

- **Input**: release id, any subset of the authored fields (artist name, title, headline, description, cover image (uploaded file), CTA text, Facebook Pixel ID, links)
- **Output (OK)**: updated release representation
- **Errors**: `release_not_found` (idempotent); `invalid_destination` (not idempotent); `invalid_facebook_pixel_id` — non-numeric Pixel ID (not idempotent; omitting the field keeps the existing value, since it can never be cleared); `invalid_cover_image_type` / `cover_image_too_large` (not idempotent); slug/URL can change after publish, previously shared links are ignored, but a warning is shown that they will break
- **Rules**: every update re-triggers landing page regeneration and republish; uploading a new cover image overwrites the existing stored image and reference; omitting it keeps the current cover unchanged
